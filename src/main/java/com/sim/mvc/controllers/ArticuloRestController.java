...
public class ArticuloRestController {
	...
	public ResponseEntity<?> update(@RequestBody Articulo articulo, BindingResult result, @PathVariable Long id) {
		...
		try {
			...
			articuloActual.setUsuario(articulo.getUsuario());
	...
	public List<Articulo> findByUsuarioId(@PathVariable Long id){
		return articuloService.findByUsuarioId(id);
	}
	...
	public ResponseEntity<?> findByTitulo(@PathVariable String titulo) {
		...
		try {
			articulo = articuloService.findByTitulo(titulo);
		} catch(DataAccessException e) {
			response.put("mensaje", "Error al realizar la consulta en la base de datos");
			response.put("error", e.getMessage().concat(": ").concat(e.getMostSpecificCause().getMessage()));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.INTERNAL_SERVER_ERROR);
		...
		if(articulo == null) {
			response.put("mensaje", "El articulo ID: ".concat(titulo.toString().concat(" no existe en la base de datos!")));
			return new ResponseEntity<Map<String, Object>>(response, HttpStatus.NOT_FOUND);
		}
